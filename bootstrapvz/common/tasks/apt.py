...
class AddDefaultSources(Task):
    ...
    def run(cls, info):
        ...
        if info.manifest.release != sid:
            info.source_lists.add('main', 'deb     {apt_security_mirror}  {system.release}/updates ' + components)
            if include_src:
                info.source_lists.add('main', 'deb-src {apt_security_mirror}  {system.release}/updates ' + components)
